...
def coerce(attr: str, is_list: bool, request_coerce: Callable[[Dict], Any]):
    """
...
class PhaseResource(BaseResource):
    ...
    def delete(self, *args, schedule_id: int, **kwargs):
        logger.error(
            "PhaseResource ignoring schedule_id=%s path parameter", schedule_id
        )
...
class PhaseListResource(BaseListResource):
    ...
    @coerce("phase_type", False, lambda x: PhaseType[x])
