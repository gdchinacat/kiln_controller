...
from .mock_service import MockService, Resource, NotFound
...
class MockServiceTest(TestCase):
    ...
    def test_walk(self):
        ...
        service = MockService({"parent": Resource({"1": parent})}, live_service=False)

        self.assertEqual([parent.resource], service.walk("/parent"))
        self.assertIs(parent.resource, service.walk("/parent/1"))
        self.assertEqual(
            [child2.resource, child3.resource], service.walk("/parent/1/child")
        )
        self.assertIs(child2.resource, service.walk("/parent/1/child/2"))
        self.assertIs(child3.resource, service.walk("/parent/1/child/3"))
