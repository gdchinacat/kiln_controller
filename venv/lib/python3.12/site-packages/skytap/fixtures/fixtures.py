...
def _clone_function(func, name):
    ...
    if hasattr(code, 'co_kwonlyargcount'):
        code = CodeType(code.co_argcount, code.co_posonlyargcount, code.co_kwonlyargcount, code.co_nlocals, code.co_stacksize, code.co_flags, code.co_code, code.co_consts, code.co_names,
                        code.co_varnames, code.co_filename, name, code.co_qualname, code.co_firstlineno, code.co_lnotab, code.co_exceptiontable, code.co_freevars, code.co_cellvars)
    else:
        raise NotImplemented("python version mismatch...do something")
...
def fixture(fixture_func, *fixture_args, **fixture_kwargs):
    ...
    def dec(func):
        ...
        def fixture_wrap(*args, **kwargs):
            ...
            # add the kwargs for the fixture
            per_call_fixture_kwargs.update(fixture_kwargs)

            # LH - can't do get(...,None) since we want to use whatever value is specified. If it's not specified,
            #     use the default if it exists.
            if 'fixture_name' in per_call_fixture_kwargs:
                # remove the fixture name from the kwargs since it isn't for the fixture
                fixture_name = per_call_fixture_kwargs.pop('fixture_name')
            else:
                fixture_name = getattr(fixture_func, 'default_fixture_name', None)

            if 'fixture_cleanup' in per_call_fixture_kwargs:
                # remove the fixture cleanup from the kwargs since it isn't for the fixture
                cleanup_func = per_call_fixture_kwargs.pop('fixture_cleanup')
            else:
                cleanup_func = getattr(fixture_func, 'default_fixture_cleanup', None)
        ...
        return fixture_wrap
        # return _clone_function(fixture_wrap,
        #                        getattr(fixture_func, "__name__",
        #                                func.__name__))
